...
extension StatusItemController {
    ...
    func applyIcon(phase: Double?) -> Bool {
           ...
           let brand = ProviderBrandIcon.image(for: primaryProvider)
        {
            if self.settings.menuBarDisplayMode == .wideProgress {
                let wideProgress = self.menuBarWideProgressModel(for: primaryProvider, snapshot: snapshot)
    ...
    func applyIcon(for provider: UsageProvider, phase: Double?) {
        ...
        {
            if self.settings.menuBarDisplayMode == .wideProgress {
                let wideProgress = self.menuBarWideProgressModel(for: provider, snapshot: snapshot)
    ...
    private static func drawWideProgressBar(
        rect: NSRect,
        window: RateWindow?,
        fillColor: NSColor,
        fillAlpha: CGFloat,
        ...
        markerColor: NSColor?)
    {
        ...
        if let window {
            ...
            let fillWidth = rect.width * CGFloat(clampedPercent / 100)
            if fillWidth > 0 {
    ...
    private static func wideProgressBarColor(hex: String) -> NSColor {
        ...
        guard let value = Int(sanitized[start...], radix: 16) else { return .darkGray }
        return NSColor(
